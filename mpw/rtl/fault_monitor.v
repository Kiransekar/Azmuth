...
module fault_monitor (
    ...
    typedef enum reg [2:0] {
        MON_IDLE = 3'b000,
        MON_MONITOR = 3'b001,
        MON_FAULT_DETECTED = 3'b010,
        MON_IRQ_ASSERT = 3'b011,
        MON_PIPELINE_HALT = 3'b100
    } monitor_state_t;

    monitor_state_t current_state, next_state;
    ...
    // ECC encoding and decoding (SECDED 72,64)
    function [71:0] ecc_encode_72_64;
        input [63:0] data_in;
        begin
            // Implement SECDED (Single Error Correction, Double Error Detection)
            // 64 data bits + 8 parity bits = 72 bits total
            ecc_encode_72_64 = {data_in, 8'h00};  // Simplified - actual implementation would compute parity
        end
    endfunction

    function [64:0] ecc_decode_72_64;  // [64:0] because it returns corrected data [63:0] + syndrome [0]
        input [71:0] data_in;
        begin
            // Implement SECDED decode
            ecc_decode_72_64 = {data_in[63:0], 1'b0};  // Simplified - actual implementation would check parity
        end
    endfunction
...
module ecc_memory #(
    ...
    output reg [1:0] error_type  // 00=none, 01=single, 10=double, 11=triple+
);

    // SECDED 72,64 requires 72-bit storage for 64-bit data
    wire [71:0] wr_data_ecc = ecc_encode_72_64(wr_data);
    reg [71:0] memory_array [0:(1<<ADDR_WIDTH)-1];
    wire [64:0] decoded_data = ecc_decode_72_64(memory_array[addr]);  // [63:0] data + [64] syndrome

    always @(posedge clk) begin
        if (wr_en) begin
            memory_array[addr] <= wr_data_ecc;
        end
        rd_data <= decoded_data[63:0];

        // Determine error type from syndrome
        if (decoded_data[64]) begin  // Syndrome indicates error
            error_type <= 2'b01;  // Single error (corrected)
            ecc_error <= 1'b1;
        end
        else begin
